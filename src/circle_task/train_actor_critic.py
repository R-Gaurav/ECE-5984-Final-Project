...
from src.circle_task.environment import Environment
...
def get_network():
  ...
  with nengo.Network() as net:
    ...
    net.probe_actor_choice = nengo.Probe(choice_node)
    net.probe_actor_aprobs = nengo.Probe(actions_prob_node)
...
if __name__=="__main__":
  ...
  parser.add_argument(
      "--sim_run_time", type=int, default=100, help="Simulation Run Time?")
  args = parser.parse_args()

  # Run the Actor-Critic model.
  net = get_network()
  sim = nengo.Simulator(net)
  sim.run(args.sim_run_time) # Execute for `sim_run_time` seconds.
